...
class HologramWindow(QWidget):
    ...
    def _ask_ollama(self):
        prompt = self.current_text or "Dame un comando útil para un asistente gestual Jarvis."
        self._log("[IA] Consultando Ollama...")
        answer = self.ollama.ask(prompt)
        self.log.append(answer[:2500])

    # ── Gestos ────────────────────────────────────────────────────────────────
    def _handle_gesture(self, state: MultiHandState):
        combo = state.combo_gesture
        ...
        main = state.main
        if main.gesture == "OPEN_HAND" and not self.active and self.frame_id % 20 == 0:
            self.active = True
            self._log("[GESTO] Mano abierta: sistema activo")

        elif main.gesture == "FIST" and self.active and self.frame_id % 20 == 0:
            self.active = False
            self._log("[GESTO] Puño: pausa")

        elif main.gesture == "PINCH" and self.active:
            if self.frame_id - self.last_pinch_frame > SETTINGS.pinch_cooldown_frames:
                self.last_pinch_frame = self.frame_id
                self._select_by_air_pointer()

        # Scroll con THREE_FINGERS (mano izquierda o principal)
        if self.active and state.left and state.left.gesture == "THREE_FINGERS":
            ix, iy = state.left.index_pos
            if hasattr(self, "_last_scroll_y"):
                delta = self._last_scroll_y - iy
                if abs(delta) > 8:
                    if self.actions.safe_mode:
                        direction = "arriba" if delta > 0 else "abajo"
                        self._log(f"[SCROLL] Gesto scroll {direction} detectado")
                    else:
                        self._run_action(lambda d=delta: self.actions.scroll(d // 8))
            self._last_scroll_y = iy
        else:
            self._last_scroll_y = None

        # Actualizar panel de gestos
        self._panel_set_line(self.gesture_panel, 0,
            f"MANOS        {len(state.hands)}")
        self._panel_set_line(self.gesture_panel, 1,
            f"RIGHT        {state.right.gesture if state.right else 'NO_HAND'}")
        self._panel_set_line(self.gesture_panel, 2,
            f"LEFT         {state.left.gesture if state.left else 'NO_HAND'}")
        self._panel_set_line(self.gesture_panel, 3,
            f"COMBO        {combo}")
    ...
    def _update_dwell(self):
        """Hover continuo sobre un botón → auto-click tras DWELL_FRAMES."""
        gp      = self.mapToGlobal(self.pointer)
        hovered = QApplication.widgetAt(gp)
        target: Optional[HoloButton] = None
        tmp = hovered
        ...
        if target is None or not self.active:
            if self._dwell_widget:
                self._dwell_widget.dwell_pct = 0.0
                self._dwell_widget.update()
            self._dwell_widget  = None
            self._dwell_frames  = 0
            self._panel_set_line(self.gesture_panel, 4, "DWELL        0 %")
            return

        if target is not self._dwell_widget:
            if self._dwell_widget:
                self._dwell_widget.dwell_pct = 0.0
                self._dwell_widget.update()
            self._dwell_widget = target
            self._dwell_frames = 0

        self._dwell_frames += 1
        pct = min(1.0, self._dwell_frames / self.DWELL_FRAMES)
        target.dwell_pct = pct
        target.update()
        self._panel_set_line(self.gesture_panel, 4,
            f"DWELL        {int(pct * 100)} %")

        if self._dwell_frames >= self.DWELL_FRAMES:
            self._log(f"[DWELL CLICK] {target.text()}")
            target.click()
            target.dwell_pct  = 0.0
            self._dwell_frames = 0
